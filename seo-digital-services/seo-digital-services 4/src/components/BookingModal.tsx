...
export const BookingModal: React.FC<BookingModalProps> = ({
  ...
  // Formspree endpoint that receives contact-form inquiries.
  // Defaults to the live form; override via VITE_FORMSPREE_ENDPOINT if needed.
  const FORMSPREE_ENDPOINT =
    (import.meta.env.VITE_FORMSPREE_ENDPOINT as string | undefined) ||
    'https://formspree.io/f/mlgqzoog';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!FORMSPREE_ENDPOINT) {
        throw new Error(
          'Contact form is not configured yet. Please email us directly and we will respond right away.'
        );
      }
      ...
      const response = await fetch(FORMSPREE_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          subject: formData.selectedPlanOrAddon,
          message: formData.message,
          _subject: `New website inquiry: ${formData.selectedPlanOrAddon}`,
        }),
      });
